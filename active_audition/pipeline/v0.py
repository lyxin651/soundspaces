"""M2 V0 orchestration; geometry and acoustic math live in their own modules."""

import hashlib
import os
import subprocess
import tempfile
from pathlib import Path
from typing import Any, Dict, Iterable, Mapping

import numpy as np
import yaml

from active_audition.acoustics.renderer import convolve_binaural, write_float32_wav
from active_audition.acoustics.rir import render_native_rir
from active_audition.config.loader import load_resolved_config
from active_audition.data.audio import load_dry_segment
from active_audition.data.catalog import load_dry_audio_registry, load_scene_registry, sha256_file
from active_audition.data.manifest import (
    read_plan_manifests,
    read_viewpoint_manifest,
    write_viewpoint_manifest,
)
from active_audition.data.storage import DatasetStorage
from active_audition.data.validation import payload_is_complete, validate_dataset
from active_audition.scene.simulator import create_scene_simulator


ACTION_ORDER = {
    "initial": 0,
    "translation": 1,
    "rotation": 2,
}


def _row_pose(row: Mapping[str, Any]) -> Any:
    from active_audition.types import ListenerPose

    base = row["base_position_world"] if "base_position_world" in row else row["snapped_base_position_world"]
    return ListenerPose(
        tuple(float(x) for x in base),
        tuple(float(x) for x in row["sensor_position_world"]),
        float(row["yaw_deg"]),
    )


def _dry_hash(dry: np.ndarray) -> str:
    return hashlib.sha256(np.asarray(dry, dtype=np.float32).tobytes()).hexdigest()


def _write_wav_atomic(storage: DatasetStorage, path: Path, waveform: np.ndarray, sample_rate_hz: int) -> None:
    fd, temp_name = tempfile.mkstemp(prefix=".wav-", suffix=".wav", dir=str(storage.root))
    os.close(fd)
    try:
        write_float32_wav(temp_name, waveform, sample_rate_hz)
        storage.atomic_write_bytes(path, Path(temp_name).read_bytes())
    finally:
        try:
            os.unlink(temp_name)
        except OSError:
            pass


def _viewpoint_row(
    episode: Mapping[str, Any],
    viewpoint_id: str,
    candidate_id: Any,
    action_type: str,
    pose: Any,
    audio_path: Path,
    rir_id: str,
    rir_path: Path,
    sample_rate_hz: int,
    waveform: np.ndarray,
    dry_hash: str,
) -> Dict[str, Any]:
    source = episode["source"]
    return {
        "episode_id": episode["episode_id"],
        "viewpoint_id": viewpoint_id,
        "candidate_id": candidate_id,
        "action_type": action_type,
        "base_position_world": list(pose.base_position_world),
        "sensor_position_world": list(pose.sensor_position_world),
        "yaw_deg": float(pose.yaw_deg),
        "audio_path": str(audio_path.relative_to(Path(episode["_dataset_root"]))),
        "rir_id": rir_id,
        "rir_path": str(rir_path.relative_to(Path(episode["_dataset_root"]))),
        "sample_rate_hz": int(sample_rate_hz),
        "num_channels": 2,
        "num_samples": int(waveform.shape[0]),
        "duration_sec": float(waveform.shape[0] / float(sample_rate_hz)),
        "dtype": str(waveform.dtype),
        "ground_truth": {
            "scene_id": episode["scene_id"],
            "source_position_world": source["position_world"],
            "dry_audio_id": source["audio_id"],
            "segment_start_sec": source["segment_start_sec"],
            "segment_duration_sec": source["segment_duration_sec"],
            "gain_db": source["gain_db"],
            "dry_hash": dry_hash,
        },
    }


def _ordered_candidate_rows(rows: Iterable[Mapping[str, Any]]) -> list:
    return sorted(
        rows,
        key=lambda row: (
            ACTION_ORDER.get(str(row.get("action_type")), 99),
            str(row.get("translation_direction") or ""),
            str(row.get("candidate_id")),
        ),
    )


def render_dataset(config_path: str, resume: bool = False) -> Dict[str, Any]:
    config = load_resolved_config(config_path)
    repo_root = Path(config["_repo_root"])
    storage = DatasetStorage.v0_debug(str(repo_root))
    storage.ensure_writable()
    manifests = read_plan_manifests(str(storage.root))
    episodes = manifests["episodes"]
    candidates = manifests["candidates"]
    if len(episodes) != 1:
        raise RuntimeError("M2 Golden render expects exactly one planned Episode")
    registry = load_dry_audio_registry(config["registries"]["dry_audio_path"], str(repo_root))
    existing = {}
    viewpoint_path = storage.manifest_path("viewpoints.jsonl")
    if viewpoint_path.exists():
        existing = {(row["episode_id"], row["viewpoint_id"]): row for row in read_viewpoint_manifest(str(storage.root))}
    rendered = []
    skipped = []
    episode_rows = []
    with create_scene_simulator(config) as context:
        for episode_input in sorted(episodes, key=lambda row: str(row["episode_id"])):
            episode = dict(episode_input)
            episode["_dataset_root"] = str(storage.root)
            source = episode["source"]
            dry = load_dry_segment(
                registry[source["audio_id"]]["path"],
                float(source["segment_start_sec"]),
                float(source["segment_duration_sec"]),
                int(config["acoustics"]["sample_rate_hz"]),
                float(source["gain_db"]),
            )
            dry_hash = _dry_hash(dry)
            episode_candidates = [
                row for row in candidates
                if row.get("episode_id") == episode["episode_id"] and bool(row.get("valid"))
            ]
            viewpoints = [("initial", None, "initial", _row_pose(episode["listener_initial"]))]
            for candidate in _ordered_candidate_rows(episode_candidates):
                viewpoints.append(
                    (
                        str(candidate["candidate_id"]),
                        str(candidate["candidate_id"]),
                        str(candidate["action_type"]),
                        _row_pose(candidate),
                    )
                )
            for viewpoint_id, candidate_id, action_type, pose in viewpoints:
                key = (episode["episode_id"], viewpoint_id)
                old = existing.get(key)
                if resume and old is not None and payload_is_complete(str(storage.root), old, bool(config["storage"]["save_rir"])):
                    rendered.append(old)
                    skipped.append(viewpoint_id)
                    continue
                rir = render_native_rir(context, source["position_world"], pose)
                rir_id = "{}__{}".format(episode["episode_id"], viewpoint_id)
                rir_path = storage.rir_cache_path(rir_id)
                if bool(config["storage"]["save_rir"]):
                    storage.atomic_write_npz(
                        rir_path,
                        {
                            "rir": rir,
                            "sample_rate_hz": np.asarray(config["acoustics"]["sample_rate_hz"], dtype=np.int64),
                            "num_samples": np.asarray(rir.shape[0], dtype=np.int64),
                        },
                    )
                waveform = convolve_binaural(dry, rir)
                audio_path = storage.viewpoint_audio_path(episode["episode_id"], viewpoint_id)
                if bool(config["storage"]["save_audio"]):
                    _write_wav_atomic(storage, audio_path, waveform, int(config["acoustics"]["sample_rate_hz"]))
                row = _viewpoint_row(
                    episode, viewpoint_id, candidate_id, action_type, pose,
                    audio_path, rir_id, rir_path, int(config["acoustics"]["sample_rate_hz"]), waveform, dry_hash,
                )
                rendered.append(row)
                episode_rows.append({"episode_id": episode["episode_id"], "viewpoint_id": viewpoint_id, "rir_samples": int(rir.shape[0]), "wav_samples": int(waveform.shape[0]), "wav_peak": float(np.max(np.abs(waveform)))} )
    manifest_path = write_viewpoint_manifest(str(storage.root), rendered)
    return {
        "dataset_root": str(storage.root),
        "viewpoints": len(rendered),
        "rendered": len(rendered) - len(skipped),
        "skipped": len(skipped),
        "manifest": str(manifest_path),
        "details": episode_rows,
    }


def finalize_dataset(config_path: str, validation: Mapping[str, Any]) -> Dict[str, Any]:
    config = load_resolved_config(config_path)
    repo_root = Path(config["_repo_root"])
    storage = DatasetStorage.v0_debug(str(repo_root))
    storage.ensure_writable()
    if validation.get("status") != "PASS":
        raise RuntimeError("cannot finalize a failed dataset")
    scenes = load_scene_registry(config["registries"]["scenes_path"], str(repo_root))
    dry_registry = load_dry_audio_registry(config["registries"]["dry_audio_path"], str(repo_root))
    config_path_obj = Path(config["_config_path"])
    config_text = config_path_obj.read_text(encoding="utf-8")
    public_config = {key: value for key, value in config.items() if not key.startswith("_")}
    storage.atomic_write_text(storage.path("config_resolved.yaml"), yaml.safe_dump(public_config, sort_keys=True))
    identity = {
        "dataset_id": "aa_v0_replica_debug_001",
        "schema": config["experiment"]["schema_version"],
        "created_at": "2026-08-25T00:00:00+08:00",
        "git_commit": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=str(repo_root), text=True).strip(),
        "config_sha256": hashlib.sha256(config_text.encode("utf-8")).hexdigest(),
        "global_seed": config["experiment"]["global_seed"],
        "habitat_version": getattr(__import__("habitat_sim"), "__version__", "unknown"),
        "scene_registry_sha256": sha256_file(Path(config["_repo_root"]) / config["registries"]["scenes_path"]),
        "dry_audio_registry_sha256": sha256_file(Path(config["_repo_root"]) / config["registries"]["dry_audio_path"]),
    }
    storage.atomic_write_json(storage.path("identity.json"), identity)
    resources = {
        "scene_assets": {scene_id: {key: value for key, value in entry.items() if key in ("scene_asset", "navmesh", "semantic_info", "stage_config")} for scene_id, entry in scenes.items()},
        "dry_audio": {audio_id: {"path": entry["path"], "sha256": entry["sha256"]} for audio_id, entry in dry_registry.items()},
    }
    storage.atomic_write_json(storage.path("resources.lock.json"), resources)
    storage.atomic_write_json(storage.path("reports", "generation_summary.json"), {"status": "PASS", "viewpoints": validation.get("viewpoint_count", 0)})
    storage.atomic_write_json(storage.path("reports", "validation.json"), validation)
    storage.atomic_write_json(storage.path("reports", "storage_summary.json"), _storage_summary(storage))
    storage.atomic_write_text(storage.path("logs", "generation.log"), "M2 generation completed\n")
    storage.atomic_write_text(storage.path("logs", "validation.log"), "M2 validation PASS\n")
    storage.atomic_write_text(storage.path("logs", "failures.jsonl"), "")
    storage.atomic_write_text(storage.success_path, "Pipeline V0 M2 finalized\n")
    return {"dataset_root": str(storage.root), "success": str(storage.success_path)}


def _storage_summary(storage: DatasetStorage) -> Mapping[str, Any]:
    files = []
    for path in sorted(storage.root.rglob("*")):
        if path.is_file() and path.name != "_SUCCESS":
            files.append({"path": str(path.relative_to(storage.root)), "bytes": path.stat().st_size})
    return {"file_count": len(files), "total_bytes": sum(row["bytes"] for row in files), "files": files}


def run_v0(config_path: str, resume: bool = False) -> Dict[str, Any]:
    from active_audition.cli import plan, precheck

    precheck_result = precheck(config_path)
    plan_result = plan(config_path)
    render_result = render_dataset(config_path, resume=resume)
    validation_result = validate_dataset(render_result["dataset_root"], load_resolved_config(config_path), require_success=False)
    finalize_result = finalize_dataset(config_path, validation_result)
    return {"precheck": precheck_result, "plan": plan_result, "render": render_result, "validation": validation_result, "finalize": finalize_result}
